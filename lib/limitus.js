...
class Limitus {
     ...
     * @return {Function}
     */
    baseCheckLimited (rule, name, key, callback) {
        ...
        this.get(key, (err, value) => {
            ...
            if (data.limited && !this._overflows) {
                return callback(new Rejected(data.info, name), data);
            }
    ...
    baseDrop (name, ident, rule, callback) {
        ...
        this.baseCheckLimited(rule, name, key, (err, data) => {
            if (err) {
            ...
            this.set(key, data.next, data.expiration, (err) => {
                ...
                if (data.limited) {
                    callback(new Rejected(data.info, name), data.info);
                } else {
...
Limitus.prototype.checkLimited = loadArgs(function (name, ident, rule, callback) {
    ...
    if (callback) {
        return this.baseCheckLimited(rule, name, key, callback);
    }
   return this.baseCheckLimitedAsync(rule, name, key);
});
